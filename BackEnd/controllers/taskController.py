...
def get_student_todo(current_user : int = Depends(RoleChecker("Student")) , db : Session = Depends(get_db)) : 
    completed_resources_ids = db.query(Task.resource_id).filter(Task.student_id == current_user.student_data.id).all()

    completed_ids = [resource[0] for resource in completed_resources_ids]


    todo_list = db.query(Resource).join(Module).join(Subscription).filter
    (Subscription.student_id == current_user.student_data.id , 
    ~Resource.id.in_(completed_ids)).all()
...
def submit_task(request : TaskRequest , current_user = Depends(RoleChecker("Student")) , db : Session = Depends(get_db)) : 
    ...
    already_done = db.query(Task).filter(Task.student_id == current_user.student_data.id , Task.resource_id == request.resource_id).first()
